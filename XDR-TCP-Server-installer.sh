#!/bin/bash
if [ "$EUID" -ne 0 ]
  then echo "Script to be run as root or sudo !"
  exit
fi
clear
echo "XDR-TCP Installer by Securistu'007/Ro"
echo "This script currently works on Ubuntu."
...
echo "Packages installed ! Downloading XDR TCP..."
	...
	cd xdrd-master
echo "XDR-TCP by default cannot run as root ! "
...
cd /home/xdrd-master
wget https://github.com/RTVDXRO/XDR-TCP-Server-installer/blob/master/XDR-TCP-Server-installer.sh
clear 
echo " "
rm README.md
rm xdr-protocol.h
